...
d3.json("ant.json").then( (location) => 
{ //Now we can do stuff, within braces.
    ...
    const projection = d3.geoAiry().scale(1000).center([172.54, -43.48])
    ...
    const antPaths = svg.selectAll(".ant-path")
        .data(location.geometry.coordinates) //52 states
        .join("path")
        .attr("class", "ant-path")
    ...
    antPaths.attr("stroke", "black")
    antPaths.attr("stroke-width", 5) //Etc.

    antPaths.attr("stroke", "")
    antPaths.attr("stroke-width", "")
